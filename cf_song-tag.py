...
class CF:
    def __init__(self):
        ...
        self.test = pd.read_json("arena_data/questions/val_hye.json", encoding='utf-8') #  곡/태그 둘 다 빈 ply는 제외한 상태
    ...
    def __call__(self, mode, song_ntop = 500, tag_ntop = 50):
        ...
        if mode == 'cf':
            ...
            val_song_res,val_tag_res,test_song_res,test_tag_res = self.mf_(train_songs_A, train_tags_A,test_songs_A, test_tags_A, 500,50,500)

        elif mode == 'multi_mf':
            res = self.multi_mf_(train_songs_A, train_tags_A,test_songs_A, test_tags_A, 500,50,100)

        elif mode == 'mixed':
            res = self.mixed_(train_songs_A, train_tags_A,test_songs_A, test_tags_A, 500,50,20)
    ...
    def mf_(self, train_songs_A, train_tags_A, test_songs_A, test_tags_A, song_ntop = 500, tag_ntop = 50, iteration=20):
        ...
        for id in tqdm(range(self.n_test_song)): # 18636 / 태그 -> 11605 행
            ...
            if (id < self.n_val_song) :  # 순서 - train, val, test
                val_song_res.append({
                "id": self.plylst_nid_id[self.plylst_test_song.index[id]],
                "songs": rec_song_idx,
                "songs_score" : rec_song_score
                })
            else:
            ...
            try :
                ...
                if (id < self.n_val_song) :
                    val_tag_res.append({
                    "id": self.plylst_nid_id[self.plylst_test_tag.index[id]],
                    "tags": rec_tag_idx,
                    "tags_score" : rec_tag_score
                    })
                else:
    ...
    def multi_mf_(self, train_songs_A, train_tags_A, test_songs_A, test_tags_A, song_ntop, tag_ntop, iteration):
        ...
        res = [{"id":self.plylst_nid_id[self.n_train + id], "songs":[self.song_sid_id.get(x) for x in rec_idx[0].tolist()],"tags":[self.tag_tid_id.get(x) for x in rec_idx[1].tolist()]} for id,rec_idx in enumerate(zip(cand_song,cand_tag))]
    ...
    def mixed_(self,train_songs_A, train_tags_A, test_songs_A, test_tags_A, song_ntop = 500, tag_ntop = 50, iteration=20):
      ...
      for r,pid in tqdm(enumerate(range(test_songs_A.shape[0]),0)):
        ...
        if self.plylst_test.loc[(self.n_train+pid),"song_dirty"] == 1:
          cand_song = als_model.recommend(pid, test_songs_A, N=song_ntop, filter_already_liked_items=False)
        ...
        if self.plylst_test.loc[(self.n_train+pid),"tag_dirty"] == 1:
          rec_tag_idx = remove_seen(tags_already,cand_tag_idx)[:tag_ntop]
